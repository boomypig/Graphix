...
function collideParticles(particle1, particle2, DT, collisionFriction) {
  ...
  const masses = [
    particle1.radius * particle1.radius,
  ...
  const combinedVelocity = new Vector(
    (initialVelocities[0].x * masses[0] + initialVelocities[1].x * masses[1]) /
      totalMass,
  ...
  const initialVelocitiesCM = [
    new Vector(
      (masses[1] / totalMass) *
        (initialVelocities[0].x - initialVelocities[1].x),
      (masses[1] / totalMass) *
        (initialVelocities[0].y - initialVelocities[1].y),
    ),
    new Vector(
      (masses[0] / totalMass) *
        (initialVelocities[1].x - initialVelocities[0].x),
      (masses[0] / totalMass) *
        (initialVelocities[1].y - initialVelocities[0].y),
    ),
  ];
  ...
  const initialVelocitiesCMTangent = [
    initialVelocitiesCM[0].x * centerOfMassTangential.x +
      initialVelocitiesCM[0].y * centerOfMassTangential.y,
    initialVelocitiesCM[1].x * centerOfMassTangential.x +
      initialVelocitiesCM[1].y * centerOfMassTangential.y,
  ];
  const initialVelocitiesCMNormal = [
    initialVelocitiesCM[0].x * centerOfMassNormal.x +
      initialVelocitiesCM[0].y * centerOfMassNormal.y,
    initialVelocitiesCM[1].x * centerOfMassNormal.x +
      initialVelocitiesCM[1].y * centerOfMassNormal.y,
  ];

  // Calculate final velocities (v values) in the Center of Mass coordinate system
  const finalVelocities = [
    new Vector(
      initialVelocitiesCMTangent[0] * centerOfMassTangential.x -
        initialVelocitiesCMNormal[0] * centerOfMassNormal.x +
        combinedVelocity.x,
      initialVelocitiesCMTangent[0] * centerOfMassTangential.y -
        initialVelocitiesCMNormal[0] * centerOfMassNormal.y +
        combinedVelocity.y,
    ),
    new Vector(
      initialVelocitiesCMTangent[1] * centerOfMassTangential.x -
        initialVelocitiesCMNormal[1] * centerOfMassNormal.x +
        combinedVelocity.x,
      initialVelocitiesCMTangent[1] * centerOfMassTangential.y -
        initialVelocitiesCMNormal[1] * centerOfMassNormal.y +
        combinedVelocity.y,
    ),
  ];
